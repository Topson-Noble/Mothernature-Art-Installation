...
public class lb_Bird : MonoBehaviour {
    ...
    public float moveDuration = 2f; // Time to reach the target position

    void Start()
    {
        // Store the initial position when the object first spawns
	...
	void fly(Vector3 target)
	{
        ...
        transform.DOMove(target, 4f)
            .OnComplete(() => DeactivateGameObject());
    }
    ...
    private void DeactivateGameObject()
    {
        ...
        // Move the GameObject back to the initial position (if desired)
        transform.position = initialPosition;// Deactivate the GameObject
    ...
    void ChangeTargetPosition()
    {
        ...
        float randomY = Random.Range(1.1f, 1.5f);

        // Update the GameObject's position while keeping the X and Z position the same
        target.transform.position = new Vector3(target.transform.position.x, randomY,target.transform.position.z);
    }
}
